...
import ImageModal from "../reuseable/ImageModal";
import Link from "next/link";
import { DeleteBlog } from "@/lib/siteApis";
// import { deleteBlog } from "@/lib/apis";

const columns = [
    { name: "CARD IMAGE", uid: "cardImage" },
    { name: "BLOG BANNER IMAGE", uid: "blogBannerImage" },
...
function ViewBlogs({ blogData }) {
    ...
    const renderCell = React.useCallback((blogData, columnKey) => {

        const cellValue = blogData[columnKey];

        switch (columnKey) {
            case "cardImage":
                return (
                    <div className="cursor-pointer">
                        <ImageModal folder={"blogs"} id={blogData?.cardImage} />
                    </div>
                );
            case "blogBannerImage":
                return (
                    <div className="cursor-pointer">
                        <ImageModal folder={"blogs"} id={blogData?.blogBannerImage} />
                    </div>
                );
            case "actions":
                return (
                    <div className="relative flex flex-col  items-start gap-2">
                        <span className="text-lg text-danger cursor-pointer active:opacity-50">
                            <Button
                                ...
                                <Link
                                    href={`/blog/blog-detail/${blogData?.slug}`}
                                >
                                    View Detail
                                </Link>
                            </Button>
                        ...
                        <span className="text-lg text-danger cursor-pointer active:opacity-50">
                            <Button
                                className="w-32  text-white bg-blue hover:bg-darkBlue">
                                <Link
                                    href={`/blog/update-blog/${blogData?.slug}`}
                                >
                                    Update
                                </Link>
                            </Button>
            ...
            <ScrollShadow
                ...
                >
                    <TableHeader>
                        {columns?.map((column) =>
                            <TableColumn
                                className="dark:bg-darkModeSecondary text-start dark:text-white text-[15px]"
                                key={column?.uid}>
                                {column?.name}
                            </TableColumn>
                        )}
